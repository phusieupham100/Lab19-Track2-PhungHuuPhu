...
Lab dành cho **AICB-P2T2 · Ngày 19 · Vector Store + Feature Store**.
Mục tiêu: build hybrid search API + Feast feature store hoàn chỉnh, rồi đo Precision@10 và P99 latency.
...
> Cả hai paths dùng **chung `qdrant-client` API và Feast definitions** — bạn có
> thể chuyển qua lại bất cứ lúc nào bằng cách đổi `QDRANT_MODE` trong `.env`.
...
**Source format:** Notebooks được lưu dưới dạng Jupytext `.py` (gọn, dễ review).
...
Lab này được thiết kế cho **vibe-coding era** theo CLI workflow: bạn dùng
AI assistant trong terminal (Claude Code, Codex CLI, OpenCode) để
generate boilerplate và tập trung vào *judgment decisions*. Đọc
[`VIBE-CODING.md`](VIBE-CODING.md) **trước khi bắt đầu NB1** (5–10 phút)
— đây là primer tổng quát, bao gồm:
